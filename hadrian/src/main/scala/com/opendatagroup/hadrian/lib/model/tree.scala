...
return com.opendatagroup.hadrian.lib.model.tree.package$SimpleComparison$.MODULE$.apply(datum, field, comparison.%s, comparison.%s, datum.fieldTypes()[field.value()], comparison.fieldTypes()[comparison.fieldIndex("value")], true, %s, %s, "%s", %s);
...
if (datum.get(field.value()) == null)
    return null;
return java.lang.Boolean.valueOf(com.opendatagroup.hadrian.lib.model.tree.package$SimpleComparison$.MODULE$.apply(datum, field, comparison.%s, comparison.%s, com.opendatagroup.hadrian.lib.model.tree.package$SimpleComparison$.MODULE$.removeNull(datum.fieldTypes()[field.value()]), comparison.fieldTypes()[comparison.fieldIndex("value")], false, %s, %s, "%s", %s));
} }).apply((com.opendatagroup.hadrian.data.PFARecord)%s, (%s)%s)""",
        javaType(paramTypes(1).asInstanceOf[AvroRecord], true, true, false),
        JVMNameMangle.s("field"),
        JVMNameMangle.s("operator"),
  ...
  class SimpleTree(val pos: Option[String] = None) extends LibFcn {
    ...
    override def javaRef(fcnType: FcnType): JavaCode =
        JavaCode("""(new %s$Walker(new scala.runtime.AbstractFunction2<com.opendatagroup.hadrian.data.PFARecord, %s, java.lang.Boolean>() {
public java.lang.Boolean apply(Object datum, Object comparison) {
return apply((com.opendatagroup.hadrian.data.PFARecord)datum, (%s)comparison);
}
public java.lang.Boolean apply(com.opendatagroup.hadrian.data.PFARecord datum, %s comparison) {
com.opendatagroup.hadrian.data.PFAEnumSymbol field = (com.opendatagroup.hadrian.data.PFAEnumSymbol)(comparison.%s);
return java.lang.Boolean.valueOf(com.opendatagroup.hadrian.lib.model.tree.package$SimpleComparison$.MODULE$.apply(datum, field, comparison.%s, comparison.%s, datum.fieldTypes()[field.value()], comparison.fieldTypes()[comparison.fieldIndex("value")], true, %s, %s, "%s", %s));
