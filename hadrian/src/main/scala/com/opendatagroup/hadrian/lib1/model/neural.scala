...
package object neural {
  ...
  object SimpleLayers extends LibFcn {
    val name = prefix + "simpleLayers"
    val sig = Sig(List(
    ...
    val doc =
      <doc>
        ...
        <error>Raises a "no layers" error if the length of model is zero.</error>
        <error>Raises a "weights, bias, or datum misaligned" error if there is any misalignment between inputs and outputs through the layers of the network.</error>
      </doc>

    def matrix_vector_mult(a: Vector[PFAArray[Double]], b: Vector[Double]): Vector[Double] = {
      for (row <- a)
      yield row.toVector zip b map Function.tupled(_*_) reduceLeft (_+_)
    }
    ...
    def apply(datum: PFAArray[Double], model: PFAArray[PFARecord], activation: (Double => Double)): PFAArray[Double] = {
      ...
      if (length == 0)
        throw new PFARuntimeException("no layers")
      ...
      while (i < (length - 1)) {
        ...
        if (bias.size != weights.size || datum.size != weights(0).size)
          throw new PFARuntimeException("weights, bias, or datum misaligned")
      ...
      val bias    = thisLayer("bias").asInstanceOf[PFAArray[Double]].toVector
      val weights = thisLayer("weights").asInstanceOf[PFAArray[PFAArray[Double]]].toVector
      if (bias.size != weights.size || x.size != weights(0).size)
        throw new PFARuntimeException("weights, bias, or datum misaligned")
