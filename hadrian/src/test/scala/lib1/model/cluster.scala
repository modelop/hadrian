...
""").head
  ...
  it must "find closest cluster with simplified signature" taggedAs(Lib1, Lib1ModelCluster) in {
    val engine = PFAEngine.fromYaml("""
input: {type: array, items: double}
output: string
cells:
...
""").head
  ...
  it must "find the closest N clusters with a simplified signature" taggedAs(Lib1, Lib1ModelCluster) in {
    val engine = PFAEngine.fromYaml("""
input: {type: array, items: double}
output: {type: array, items: string}
cells:
