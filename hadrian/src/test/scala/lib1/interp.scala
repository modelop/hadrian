...
package test.scala.lib1.interp
...
class Lib1InterpSuite extends FlatSpec with Matchers {
  "interp nearest" must "pick the closest item in 1-d" taggedAs(Lib1, Lib1Interp) in {
...
""").head
  ...
  it must "pick the closest item in N-d" taggedAs(Lib1, Lib1Interp) in {
    val engine = PFAEngine.fromYaml("""
input: double
output: int
cells:
...
""").head
  ...
  it must "pick the closest item in an abstract metric" taggedAs(Lib1, Lib1Interp) in {
    val engine = PFAEngine.fromYaml("""
input: string
output: int
cells:
...
""").head
  ...
  "interp linear" must "interpolate scalars in 1-d" taggedAs(Lib1, Lib1Interp) in {
    val engine = PFAEngine.fromYaml("""
input: double
output: double
cells:
...
""").head
  ...
  it must "interpolate vectors in 1-d" taggedAs(Lib1, Lib1Interp) in {
    val engine = PFAEngine.fromYaml("""
input: double
output:
  type: array
...
""").head
  ...
  it must "interpolate with flat extremes" taggedAs(Lib1, Lib1Interp) in {
    val engine = PFAEngine.fromYaml("""
input: double
output: double
cells:
...
""").head
  ...
  it must "interpolate with missing extremes" taggedAs(Lib1, Lib1Interp) in {
    val engine = PFAEngine.fromYaml("""
input: double
output: ["null", double]
cells:
