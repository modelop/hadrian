...
class Lib1ParseSuite extends FlatSpec with Matchers {
  "parse" must "parse int" taggedAs(Lib1, Lib1Parse) in {
...
""").head
  ...
  it must "parse long" taggedAs(Lib1, Lib1Parse) in {
    val engine = PFAEngine.fromYaml("""
input: string
output: long
...
""").head
  ...
  it must "parse float" taggedAs(Lib1, Lib1Parse) in {
    val engine = PFAEngine.fromYaml("""
input: string
output: float
...
""").head
  ...
  it must "parse double" taggedAs(Lib1, Lib1Parse) in {
    val engine = PFAEngine.fromYaml("""
input: string
output: double
