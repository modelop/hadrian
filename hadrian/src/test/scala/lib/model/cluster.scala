...
package test.scala.lib.model.cluster
...
class LibModelClusterSuite extends FlatSpec with Matchers {
  "cluster model" must "find closest cluster" taggedAs(Lib, LibModelCluster) in {
...
""").head
  ...
  it must "find closest cluster with strings" taggedAs(Lib, LibModelCluster) in {
    val engine = PFAEngine.fromYaml("""
input: string
output: string
cells:
...
""").head
    ...
    engine.action("") should be ("a")
    engine.action("b") should be ("a")
    engine.action("bb") should be ("aa")
    engine.action("bbb") should be ("aaa")
    engine.action("bbbb") should be ("aaaa")
    engine.action("bbbbb") should be ("aaaaa")
    engine.action("bbbbbb") should be ("aaaaa")
  }

  it must "find closest cluster with simplified signature" taggedAs(Lib, LibModelCluster) in {
    val engine = PFAEngine.fromYaml("""
input: {type: array, items: double}
output: string
cells:
...
""").head
  ...
  it must "find the closest N clusters" taggedAs(Lib, LibModelCluster) in {
    val engine = PFAEngine.fromYaml("""
input: {type: array, items: double}
...
""").head
  ...
  it must "find closest N clusters with strings" taggedAs(Lib, LibModelCluster) in {
    val engine = PFAEngine.fromYaml("""
input: string
output: {type: array, items: string}
cells:
...
""").head
    ...
    engine.action("").asInstanceOf[PFAArray[String]].toVector should be (Vector("a", "aa", "aaa"))
    engine.action("b").asInstanceOf[PFAArray[String]].toVector should be (Vector("a", "aa", "aaa"))
    engine.action("bb").asInstanceOf[PFAArray[String]].toVector should be (Vector("aa", "a", "aaa"))
    engine.action("bbb").asInstanceOf[PFAArray[String]].toVector should be (Vector("aaa", "aa", "aaaa"))
    engine.action("bbbb").asInstanceOf[PFAArray[String]].toVector should be (Vector("aaaa", "aaa", "aaaaa"))
    engine.action("bbbbb").asInstanceOf[PFAArray[String]].toVector should be (Vector("aaaaa", "aaaa", "aaa"))
    engine.action("bbbbbb").asInstanceOf[PFAArray[String]].toVector should be (Vector("aaaaa", "aaaa", "aaa"))
  }

  it must "find the closest N clusters with a simplified signature" taggedAs(Lib, LibModelCluster) in {
    val engine = PFAEngine.fromYaml("""
input: {type: array, items: double}
output: {type: array, items: string}
cells:
...
""").head
  ...
  it must "create some random seeds" taggedAs(Lib, LibModelCluster) in {
    val engine = PFAEngine.fromYaml("""
input: int
output:
  type: array
...
""").head
    ...
    intercept[PFARuntimeException] { engine.action(java.lang.Integer.valueOf(6)) }
  }
