...
package test.scala.lib.model.naive
...
class LibModelNaiveSuite extends FlatSpec with Matchers {
  "model.bayes.gaussian" must "do array signature" taggedAs(Lib, LibModelNaive) in {
...
""").head
  ...
  it must "do the map signature too" taggedAs(Lib, LibModelNaive) in {
    val engine = PFAEngine.fromYaml("""
...
""").head
  ...
  "model.naive.multinomial" must "do array signature" taggedAs(Lib, LibModelNaive) in {
    val engine = PFAEngine.fromYaml("""
input: {type: array, items: double}
output: {type: array, items: double}
cells:
...
""").head
  ...
  "model.naive.bernoulli" must "do array(string) signature" taggedAs(Lib, LibModelNaive) in {
    val engine = PFAEngine.fromYaml("""
