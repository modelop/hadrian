...
class TypeCheckSuite extends FlatSpec with Matchers {
  ...
  it must "test new" taggedAs(TypeCheck) in {
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"new": [1, 2, ["three"]], "type": {"type": "array", "items": "int"}}""")) } 
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"new": {"one": 1, "two": 2, "three": ["three"]}, "type": {"type": "map", "values": "int"}}""")) } 
  ...
  it must "test let" taggedAs(TypeCheck) in {
    ...
    an [PFASyntaxException] should be thrownBy { inferType(jsonToAst.expr("""{"let": {}}""")) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"let": {"y": {"let": {"x": 12}}}}""")) } 
    inferType(jsonToAst.expr("""{"let": {"y": {"do": {"let": {"x": 12}}}}}"""))
    inferType(jsonToAst.expr("""{"do": [{"let": {"y": {"do": {"let": {"x": 12}}}}}, "y"]}""")) should be (AvroNull())
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"do": [{"let": {"x": 12}}, {"let": {"x": 12}}]}""")) } 
  }
  ...
  it must "test set" taggedAs(TypeCheck) in {
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"set": {"x": 999}}""")) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"do": [{"set": {"x": 999}}]}""")) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"do": [{"set": {"x": 999}}, {"let": {"x": 12}}]}""")) } 
    an [PFASyntaxException] should be thrownBy { inferType(jsonToAst.expr("""{"set": {}}""")) } 
    an [PFASyntaxException] should be thrownBy { inferType(jsonToAst.expr("""{"do": [{"let": {"x": 12}}, {"set": {}}]}""")) } 
    inferType(jsonToAst.expr("""{"do": [{"let": {"x": 12.4}}, {"set": {"x": 999}}, "x"]}""")) should be (AvroDouble())
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"do": [{"let": {"x": 12}}, {"set": {"x": 99.9}}, "x"]}""")) } 
  }

  it must "test ref" taggedAs(TypeCheck) in {
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr(""""x"""")) } 
    inferType(jsonToAst.expr(""""x""""), Map("x" -> AvroInt())) should be (AvroInt())
  }

  it must "test attr" taggedAs(TypeCheck) in {
    an [PFASyntaxException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": []}"""), Map("x" -> AvroArray(AvroInt()))) } 
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [1]}"""), Map("x" -> AvroInt())) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [1, 2, 3, 4]}"""), Map("x" -> AvroArray(AvroArray(AvroArray(AvroInt()))))) } 
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [["one"], ["two"], ["three"], ["four"]]}"""), Map("x" -> AvroMap(AvroMap(AvroMap(AvroInt()))))) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [["one"]]}"""), Map("x" -> AvroInt())) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [2]}"""), Map("x" -> AvroMap(AvroInt()))) } 
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [["one"], 1, ["one"], 1]}"""), Map("x" -> AvroMap(AvroArray(AvroArray(AvroMap(AvroInt())))))) } 
    inferType(jsonToAst.expr("""{"attr": "x", "path": [["one"]]}"""), Map("x" -> AvroRecord(List(AvroField("one", AvroInt())), "MyRecord"))) should be (AvroInt())
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": ["y"]}"""), Map("x" -> AvroRecord(List(AvroField("one", AvroInt())), "MyRecord"), "y" -> AvroString())) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [["two"]]}"""), Map("x" -> AvroRecord(List(AvroField("one", AvroInt())), "MyRecord"))) } 
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [["one"], 1, "y", 1]}"""), Map("x" -> AvroMap(AvroArray(AvroRecord(List(AvroField("one", AvroArray(AvroInt()))), "MyRecord"))), "y" -> AvroString())) } 
  }
  ...
  it must "test attr-to" taggedAs(TypeCheck) in {
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [2], "to": 12.4}"""), Map("x" -> AvroArray(AvroInt()))) } 

    inferType(jsonToAst.expr("""{"attr": "x", "path": [2], "to": {"params": [{"x": "int"}], "ret": "int", "do": "x"}}"""), Map("x" -> AvroArray(AvroInt()))) should be (AvroArray(AvroInt()))
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [2], "to": {"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}}"""), Map("x" -> AvroArray(AvroInt()))) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [2], "to": {"params": [{"x": "string"}], "ret": "int", "do": 12}}"""), Map("x" -> AvroArray(AvroInt()))) } 
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [2], "to": {"fcn": "u.f"}}"""), Map("x" -> AvroArray(AvroInt())), fcns = Map("u.f" -> UserFcn.fromFcnDef("u.f", jsonToAst.fcn("""{"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}""")))) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"attr": "x", "path": [2], "to": {"fcn": "u.f"}}"""), Map("x" -> AvroArray(AvroInt())), fcns = Map("u.f" -> UserFcn.fromFcnDef("u.f", jsonToAst.fcn("""{"params": [{"x": "string"}], "ret": "int", "do": 12}""")))) } 
  }
  ...
  it must "test cell" taggedAs(TypeCheck) in {
    for (shared <- List(true, false)) {
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [1]}"""), cells = Map("x" -> Cell(AvroInt(), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroInt()), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [1, 2, 3, 4]}"""), cells = Map("x" -> Cell(AvroArray(AvroArray(AvroArray(AvroInt()))), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroArray(AvroArray(AvroArray(AvroInt())))), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [["one"], ["two"], ["three"], ["four"]]}"""), cells = Map("x" -> Cell(AvroMap(AvroMap(AvroMap(AvroInt()))), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroMap(AvroMap(AvroMap(AvroInt())))), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [["one"]]}"""), cells = Map("x" -> Cell(AvroInt(), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroInt()), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [2]}"""), cells = Map("x" -> Cell(AvroMap(AvroInt()), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroMap(AvroInt())), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [["one"], 1, ["one"], 1]}"""), cells = Map("x" -> Cell(AvroMap(AvroArray(AvroArray(AvroMap(AvroInt())))), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroMap(AvroArray(AvroArray(AvroMap(AvroInt()))))), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": ["y"]}"""), Map("y" -> AvroString()), cells = Map("x" -> Cell(AvroRecord(List(AvroField("one", AvroInt())), "MyRecord"), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroRecord(List(AvroField("one", AvroInt())), "MyRecord")), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [["two"]]}"""), cells = Map("x" -> Cell(AvroRecord(List(AvroField("one", AvroInt())), "MyRecord"), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroRecord(List(AvroField("one", AvroInt())), "MyRecord")), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [["one"], 1, "y", 1]}"""), Map("y" -> AvroString()), cells = Map("x" -> Cell(AvroMap(AvroArray(AvroRecord(List(AvroField("one", AvroArray(AvroInt()))), "MyRecord"))), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroMap(AvroArray(AvroRecord(List(AvroField("one", AvroArray(AvroInt()))), "MyRecord")))), shared, false, CellPoolSource.EMBEDDED))) } 
    }
  }
  ...
  it must "test cell-to" taggedAs(TypeCheck) in {
    for (shared <- List(true, false)) {
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [2], "to": 12.4}"""), cells = Map("x" -> Cell(AvroArray(AvroInt()), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [2], "to": {"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}}"""), cells = Map("x" -> Cell(AvroArray(AvroInt()), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [2], "to": {"params": [{"x": "string"}], "ret": "int", "do": 12}}"""), cells = Map("x" -> Cell(AvroArray(AvroInt()), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cell": "x", "path": [2], "to": {"fcn": "u.f"}}"""), cells = Map("x" -> Cell(AvroArray(AvroInt()), EmbeddedJsonDomCellSource(JsonArray(Seq()), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED)), fcns = Map("u.f" -> UserFcn.fromFcnDef("u.f", jsonToAst.fcn("""{"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}""")))) } 
  ...
  it must "test pool" taggedAs(TypeCheck) in {
    for (shared <- List(true, false)) {
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 1]}"""), pools = Map("x" -> Pool(AvroInt(), EmbeddedJsonDomPoolSource(Map(), AvroInt()), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 1, 2, 3, 4]}"""), pools = Map("x" -> Pool(AvroArray(AvroArray(AvroArray(AvroInt()))), EmbeddedJsonDomPoolSource(Map(), AvroArray(AvroArray(AvroArray(AvroInt())))), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], ["one"], ["two"], ["three"], ["four"]]}"""), pools = Map("x" -> Pool(AvroMap(AvroMap(AvroMap(AvroInt()))), EmbeddedJsonDomPoolSource(Map(), AvroMap(AvroMap(AvroMap(AvroInt())))), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], ["one"]]}"""), pools = Map("x" -> Pool(AvroInt(), EmbeddedJsonDomPoolSource(Map(), AvroInt()), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 2]}"""), pools = Map("x" -> Pool(AvroMap(AvroInt()), EmbeddedJsonDomPoolSource(Map(), AvroMap(AvroInt())), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], ["one"], 1, ["one"], 1]}"""), pools = Map("x" -> Pool(AvroMap(AvroArray(AvroArray(AvroMap(AvroInt())))), EmbeddedJsonDomPoolSource(Map(), AvroMap(AvroArray(AvroArray(AvroMap(AvroInt()))))), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], "y"]}"""), Map("y" -> AvroString()), pools = Map("x" -> Pool(AvroRecord(List(AvroField("one", AvroInt())), "MyRecord"), EmbeddedJsonDomPoolSource(Map(), AvroRecord(List(AvroField("one", AvroInt())), "MyRecord")), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], ["two"]]}"""), pools = Map("x" -> Pool(AvroRecord(List(AvroField("one", AvroInt())), "MyRecord"), EmbeddedJsonDomPoolSource(Map(), AvroRecord(List(AvroField("one", AvroInt())), "MyRecord")), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], ["one"], 1, "y", 1]}"""), Map("y" -> AvroString()), pools = Map("x" -> Pool(AvroMap(AvroArray(AvroRecord(List(AvroField("one", AvroArray(AvroInt()))), "MyRecord"))), EmbeddedJsonDomPoolSource(Map(), AvroMap(AvroArray(AvroRecord(List(AvroField("one", AvroArray(AvroInt()))), "MyRecord")))), shared, false, CellPoolSource.EMBEDDED))) } 
    }
  }
  ...
  it must "test pool-to" taggedAs(TypeCheck) in {
    for (shared <- List(true, false)) {
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 2], "to": 12.4, "init": {"value": [999.9], "type": {"type": "array", "items": "double"}}}"""), pools = Map("x" -> Pool(AvroArray(AvroInt()), EmbeddedJsonDomPoolSource(Map(), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED))) } 

      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 2], "to": {"params": [{"x": "int"}], "ret": "int", "do": "x"}, "init": {"value": [999.9], "type": {"type": "array", "items": "double"}}}"""), pools = Map("x" -> Pool(AvroArray(AvroInt()), EmbeddedJsonDomPoolSource(Map(), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 2], "to": {"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}"""), pools = Map("x" -> Pool(AvroArray(AvroInt()), EmbeddedJsonDomPoolSource(Map(), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 2], "to": {"params": [{"x": "string"}], "ret": "int", "do": 12}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}"""), pools = Map("x" -> Pool(AvroArray(AvroInt()), EmbeddedJsonDomPoolSource(Map(), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED))) } 

      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 2], "to": {"fcn": "u.f"}, "init": {"value": [999.9], "type": {"type": "array", "items": "double"}}}"""), pools = Map("x" -> Pool(AvroArray(AvroInt()), EmbeddedJsonDomPoolSource(Map(), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED)), fcns = Map("u.f" -> UserFcn.fromFcnDef("u.f", jsonToAst.fcn("""{"params": [{"x": "int"}], "ret": "int", "do": "x"}""")))) } 
      ...
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 2], "to": {"fcn": "u.f"}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}"""), pools = Map("x" -> Pool(AvroArray(AvroInt()), EmbeddedJsonDomPoolSource(Map(), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED)), fcns = Map("u.f" -> UserFcn.fromFcnDef("u.f", jsonToAst.fcn("""{"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}""")))) } 
      an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"pool": "x", "path": [["p"], 2], "to": {"fcn": "u.f"}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}"""), pools = Map("x" -> Pool(AvroArray(AvroInt()), EmbeddedJsonDomPoolSource(Map(), AvroArray(AvroInt())), shared, false, CellPoolSource.EMBEDDED)), fcns = Map("u.f" -> UserFcn.fromFcnDef("u.f", jsonToAst.fcn("""{"params": [{"x": "string"}], "ret": "int", "do": 12}""")))) } 
    }
  }
  ...
  it must "test if" taggedAs(TypeCheck) in {
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"if": null, "then": 12}""")) } 
  }
  ...
  it must "test cond" taggedAs(TypeCheck) in {
    ...
    an [PFASyntaxException] should be thrownBy { inferType(jsonToAst.expr("""{"cond": []}""")) } 
    an [PFASyntaxException] should be thrownBy { inferType(jsonToAst.expr("""{"cond": [], "else": 999}""")) } 
  }

  it must "test while" taggedAs(TypeCheck) in {
    inferType(jsonToAst.expr("""{"while": true, "do": 12}""")) should be (AvroNull())
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"while": null, "do": 12}""")) } 
  }

  it must "test do-until" taggedAs(TypeCheck) in {
    inferType(jsonToAst.expr("""{"do": 12, "until": true}""")) should be (AvroNull())
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"do": 12, "until": null}""")) } 
  }
  ...
  it must "test for" taggedAs(TypeCheck) in {
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"for": {"x": 0}, "while": true, "step": {"x": 1}, "do": "y"}""")) } 
    an [PFASyntaxException] should be thrownBy { inferType(jsonToAst.expr("""{"for": {}, "while": true, "step": {"x": 1}, "do": 12}""")) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"for": {"x": 0}, "while": null, "step": {"x": 1}, "do": 12}""")) } 
    an [PFASyntaxException] should be thrownBy { inferType(jsonToAst.expr("""{"for": {"x": 0}, "while": true, "step": {}, "do": 12}""")) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"for": {"x": 0}, "while": true, "step": {"y": 1}, "do": 12}""")) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"for": {"x": 0}, "while": true, "step": {"x": 2.2}, "do": 12}""")) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"for": {"x": 0}, "while": true, "step": {"x": 1}, "do": 12}"""), Map("x" -> AvroInt())) } 
    inferType(jsonToAst.expr("""{"for": {"x": 0}, "while": true, "step": {"x": 1}, "do": {"set": {"x": 12}}}""")) should be (AvroNull())
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"for": {"x": 0}, "while": true, "step": {"x": 1}, "do": {"set": {"x": 12.4}}}""")) } 
  }
  ...
  it must "test foreach" taggedAs(TypeCheck) in {
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"foreach": "x", "in": {"type": {"type": "map", "values": "int"}, "value": {"one": 1, "two": 2, "three": 3}}, "do": 12}""")) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"foreach": "x", "in": {"type": {"type": "array", "items": "int"}, "value": [1, 2, 3]}, "do": 12}"""), Map("x" -> AvroInt())) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"foreach": "x", "in": {"type": {"type": "array", "items": "int"}, "value": [1, 2, 3]}, "do": "y"}""")) } 
    inferType(jsonToAst.expr("""{"foreach": "x", "in": {"type": {"type": "array", "items": "int"}, "value": [1, 2, 3]}, "do": {"set": {"x": 12}}}""")) should be (AvroNull())
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"foreach": "x", "in": {"type": {"type": "array", "items": "int"}, "value": [1, 2, 3]}, "do": {"set": {"x": 12.4}}}""")) } 
  }
  ...
  it must "test forkeyval" taggedAs(TypeCheck) in {
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"forkey": "k", "forval": "v", "in": {"type": {"type": "array", "items": "int"}, "value": [1, 2, 3]}, "do": 12}""")) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"forkey": "k", "forval": "v", "in": {"type": {"type": "map", "values": "int"}, "value": {"one": 1, "two": 2, "three": 3}}, "do": 12}"""), Map("k" -> AvroString())) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"forkey": "k", "forval": "v", "in": {"type": {"type": "map", "values": "int"}, "value": {"one": 1, "two": 2, "three": 3}}, "do": 12}"""), Map("v" -> AvroInt())) } 
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"forkey": "k", "forval": "v", "in": {"type": {"type": "map", "values": "int"}, "value": {"one": 1, "two": 2, "three": 3}}, "do": "z"}""")) } 
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"forkey": "k", "forval": "v", "in": {"type": {"type": "map", "values": "int"}, "value": {"one": 1, "two": 2, "three": 3}}, "do": {"set": {"k": 12}}}""")) } 
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"forkey": "k", "forval": "v", "in": {"type": {"type": "map", "values": "int"}, "value": {"one": 1, "two": 2, "three": 3}}, "do": {"set": {"v": ["hello"]}}}""")) } 
  }
  ...
  it must "test cast-case" taggedAs(TypeCheck) in {
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cast": "x", "cases": [{"as": "int", "named": "xi", "do": 12}, {"as": "string", "named": "xs", "do": 999}, {"as": "bytes", "named": "xb", "do": 999}]}"""), Map("x" -> AvroUnion(List(AvroInt(), AvroString())))) } 

    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"cast": "x", "cases": [{"as": "int", "named": "xi", "do": 12}, {"as": "string", "named": "xs", "do": 999}]}"""), Map("x" -> AvroUnion(List(AvroInt(), AvroString(), AvroBytes())))) } 
  ...
  it must "test upcast" taggedAs(TypeCheck) in {
    ...
    an [PFASemanticException] should be thrownBy { inferType(jsonToAst.expr("""{"upcast": "x", "as": "int"}"""), Map("x" -> AvroDouble())) } 
  }
