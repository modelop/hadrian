...
class TestTypeCheck(unittest.TestCase):
    ...
    def testCell(self):
        for shared in True, False:
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [1]}'''), cells={"x": Cell(AvroInt(), "", shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [1, 2, 3, 4]}'''), cells={"x": Cell(AvroArray(AvroArray(AvroArray(AvroInt()))), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [["one"], ["two"], ["three"], ["four"]]}'''), cells={"x": Cell(AvroMap(AvroMap(AvroMap(AvroInt()))), "", shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [["one"]]}'''), cells={"x": Cell(AvroInt(), "", shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [2]}'''), cells={"x": Cell(AvroMap(AvroInt()), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [["one"], 1, ["one"], 1]}'''), cells={"x": Cell(AvroMap(AvroArray(AvroArray(AvroMap(AvroInt())))), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": ["y"]}'''), {"y": AvroString()}, cells={"x": Cell(AvroRecord([AvroField("one", AvroInt())], "MyRecord"), "", shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [["two"]]}'''), cells={"x": Cell(AvroRecord([AvroField("one", AvroInt())], "MyRecord"), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [["one"], 1, "y", 1]}'''), {"y": AvroString()}, cells={"x": Cell(AvroMap(AvroArray(AvroRecord([AvroField("one", AvroArray(AvroInt()))], "MyRecord"))), "", shared, False)}))
    ...
    def testCellTo(self):
        for shared in True, False:
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [2], "to": 12.4}'''), cells={"x": Cell(AvroArray(AvroInt()), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [2], "to": {"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}}'''), cells={"x": Cell(AvroArray(AvroInt()), "", shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [2], "to": {"params": [{"x": "string"}], "ret": "int", "do": 12}}'''), cells={"x": Cell(AvroArray(AvroInt()), "", shared, False)}))

            self.assertTrue(self.typeEquality(inferType(jsonToAst.expr('''{"cell": "x", "path": [2], "to": {"fcn": "u.f"}}'''), cells={"x": Cell(AvroArray(AvroInt()), "", shared, False)}, fcns={"u.f": UserFcn.fromFcnDef("u.f", jsonToAst.fcn('''{"params": [{"x": "int"}], "ret": "int", "do": "x"}'''))}), AvroArray(AvroInt())))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [2], "to": {"fcn": "u.f"}}'''), cells={"x": Cell(AvroArray(AvroInt()), "", shared, False)}, fcns={"u.f": UserFcn.fromFcnDef("u.f", jsonToAst.fcn('''{"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}'''))}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"cell": "x", "path": [2], "to": {"fcn": "u.f"}}'''), cells={"x": Cell(AvroArray(AvroInt()), "", shared, False)}, fcns={"u.f": UserFcn.fromFcnDef("u.f", jsonToAst.fcn('''{"params": [{"x": "string"}], "ret": "int", "do": 12}'''))}))

    def testPool(self):
        for shared in True, False:
            self.assertRaises(PFASyntaxException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": []}'''), pools={"x": Pool(AvroArray(AvroInt()), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 1]}'''), pools={"x": Pool(AvroInt(), "", shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 1, 2, 3, 4]}'''), pools={"x": Pool(AvroArray(AvroArray(AvroArray(AvroInt()))), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], ["one"], ["two"], ["three"], ["four"]]}'''), pools={"x": Pool(AvroMap(AvroMap(AvroMap(AvroInt()))), "", shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], ["one"]]}'''), pools={"x": Pool(AvroInt(), "", shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2]}'''), pools={"x": Pool(AvroMap(AvroInt()), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], ["one"], 1, ["one"], 1]}'''), pools={"x": Pool(AvroMap(AvroArray(AvroArray(AvroMap(AvroInt())))), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], "y"]}'''), {"y": AvroString()}, pools={"x": Pool(AvroRecord([AvroField("one", AvroInt())], "MyRecord"), "", shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], ["two"]]}'''), pools={"x": Pool(AvroRecord([AvroField("one", AvroInt())], "MyRecord"), "", shared, False)}))
            ...
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], ["one"], 1, "y", 1]}'''), {"y": AvroString()}, pools={"x": Pool(AvroMap(AvroArray(AvroRecord([AvroField("one", AvroArray(AvroInt()))], "MyRecord"))), "", shared, False)}))

    def testPoolTo(self):
        for shared in True, False:
            self.assertTrue(self.typeEquality(inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": 12, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}), AvroArray(AvroInt())))
            self.assertRaises(PFASemanticException, lambda: self.assertTrue(self.typeEquality(inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": 12.4, "init": {"value": [999.9], "type": {"type": "array", "items": "double"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}), AvroNull())))

            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": {"params": [{"x": "int"}], "ret": "int", "do": "x"}, "init": {"value": [999.9], "type": {"type": "array", "items": "double"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}))
            self.assertTrue(self.typeEquality(inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": {"params": [{"x": "int"}], "ret": "int", "do": "x"}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}), AvroArray(AvroInt())))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": {"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": {"params": [{"x": "string"}], "ret": "int", "do": 12}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}))

            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": {"fcn": "u.f"}, "init": {"value": [999.9], "type": {"type": "array", "items": "double"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}, fcns={"u.f": UserFcn.fromFcnDef("u.f", jsonToAst.fcn('''{"params": [{"x": "int"}], "ret": "int", "do": "x"}'''))}))
            self.assertTrue(self.typeEquality(inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": {"fcn": "u.f"}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}, fcns={"u.f": UserFcn.fromFcnDef("u.f", jsonToAst.fcn('''{"params": [{"x": "int"}], "ret": "int", "do": "x"}'''))}), AvroArray(AvroInt())))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": {"fcn": "u.f"}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}, fcns={"u.f": UserFcn.fromFcnDef("u.f", jsonToAst.fcn('''{"params": [{"x": "int"}], "ret": "string", "do": [["hello"]]}'''))}))
            self.assertRaises(PFASemanticException, lambda: inferType(jsonToAst.expr('''{"pool": "x", "path": [["p"], 2], "to": {"fcn": "u.f"}, "init": {"value": [999], "type": {"type": "array", "items": "int"}}}'''), pools={"x": Pool(AvroArray(AvroInt()), {}, shared, False)}, fcns={"u.f": UserFcn.fromFcnDef("u.f", jsonToAst.fcn('''{"params": [{"x": "string"}], "ret": "int", "do": 12}'''))}))
