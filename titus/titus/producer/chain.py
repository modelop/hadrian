    ...
    for i, pfa in enumerate(pfas):
        ...
        def genericReplacer(expr, self):
            if isinstance(expr, FcnDef):
                return FcnDef([{t.keys()[0]: newPlaceholder(i, list(t.values())[0])} for t in expr.params],
        ...
        for fcnName, fcnDef in pfa.fcns.items():
            ...
            fcns[prefixFcnDef(i, pfa, fcnName)] = FcnDef([{t.keys()[0]: newPlaceholder(i, list(t.values())[0])} for t in fcnDef.paramsPlaceholder],
