...
output = record("Output", key=string, value=record("Value", mass=double, radius=double, max_distance=double))
output.fields["value"].fields["name"] = string   # oops, couldn't add it at first because "Value" is the name!  (and I didn't feel like using the JSON form, as above)
...
def action(star):
    for planet in star.planets:
        if planet.mass is not None and planet.radius is not None and planet.max_distance is not None:
            emit({"key": star.name, "value": Value(planet.name, planet.mass, planet.radius, planet.max_distance)})
