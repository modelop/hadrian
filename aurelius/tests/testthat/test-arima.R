...
test_that("check different specifications of ARIMA models", {
  ...
  # test a model with external regressors
  time <- as.numeric(time(LakeHuron) - 1920)
  input <- time[length(time)]+(1:10)
  model <- arima(LakeHuron, order = c(2,0,0), xreg = time)
  as_pfa <- pfa(model)
  engine <- pfa_engine(as_pfa)
  expect_equal(unlist(engine$action(list(h=3, xreg=list(time=input)))), 
